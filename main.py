...
def place_enemies_on_map(enemies, game_map):
    positions = [(1, 2), (2, 4), (len(game_map[0])-2, len(game_map)-2)]  # Фиксированные позиции для врагов
...
def generate_enemies(width, height, num_enemies):
    enemies = []
    enemy_types = ['орк', 'гоблин', 'тролль']
    
    for i in range(num_enemies):
        if map_type != 'basement':
            enemy_type = enemy_types[i % len(enemy_types)]  # циклически выбираем тип
